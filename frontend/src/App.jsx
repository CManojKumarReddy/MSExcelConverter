...
function LoginScreen({ onSuccess, onError, error, loading }) {
  return (
    <div className="login-screen">
      <div className="login-card">
        ...
        <h1 className="login-title">MSExcelConverter</h1>
        <p className="login-subtitle">Any doc to EXCEL converter</p>
        <p className="login-desc">Sign in to start converting your documents into Excel spreadsheets instantly.</p>
        ...
        <p className="login-footer">
          Supports PDF, DOCX, PNG, JPG, CSV and TXT files
        </p>
      </div>
    </div>
  )
...
function loadStoredAuth() {
  try {
    const raw = localStorage.getItem('msec_auth')
    if (!raw) return null
...
export default function App() {
  ...
  // Auth state
  const [auth, setAuth] = useState(() => loadStoredAuth())   // { token, user } or null
  ...
  const handleGoogleCredential = useCallback(async (credentialResponse) => {
    ...
    try {
      ...
      localStorage.setItem('msec_auth', JSON.stringify(authData))
      setAuth(authData)
    } catch (err) {
      setAuthError(err.response?.data?.detail || 'Sign-in failed. Please try again.')
    } finally {
  ...
  const handleSignOut = useCallback(() => {
    localStorage.removeItem('msec_auth')
    setAuth(null)
    setMessages([])
    setPendingFile(null)
    setPasswordInput('')
  ...
  }, [sheetMode, mergeImageCols, adminMode, useAzure, auth])
  ...
  if (!auth) {
    return (
      <LoginScreen
        ...
        onError={() => { setAuthError('Google sign-in failed. Please try again.'); setAuthLoading(false) }}
        error={authError}
        loading={authLoading}
      />
    )
      ...
      <header className="header">
        ...
        <div className="header__badges">
          ...
          <div className="user-badge">
            {auth.user.picture
              ? <img className="user-badge__avatar" src={auth.user.picture} alt={auth.user.name} referrerPolicy="no-referrer" />
              : <span className="user-badge__initials">{auth.user.name?.[0] ?? '?'}</span>
            }
            <span className="user-badge__name">{auth.user.name || auth.user.email}</span>
            <button className="user-badge__signout" onClick={handleSignOut} title="Sign out">
              Sign out
            </button>
          </div>
        </div>
